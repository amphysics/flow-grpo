...
def main(_):
    ...
    reward_fn = getattr(flow_grpo.rewards, 'pickscore_score')(accelerator.device, config.reward_fn)
    eval_reward_fn = getattr(flow_grpo.rewards, 'pickscore_score')(accelerator.device, config.reward_fn)
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        for i in tqdm(
            ...
            # yield to to make sure reward computation starts
            time.sleep(0)
            # here we use append to append the rewards to the samples
            samples.append(
                {
